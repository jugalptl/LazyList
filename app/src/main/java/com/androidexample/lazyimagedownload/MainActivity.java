...
public class MainActivity extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        Cursor cursor = getContentResolver().query(MediaStore.Images.Media.EXTERNAL_CONTENT_URI,new String[]{MediaStore.Images.Media.DATA,MediaStore.Images.Media._ID},null,null,MediaStore.Images.Media._ID);
        Toast.makeText(getApplicationContext(),String.valueOf(cursor.getCount()),Toast.LENGTH_LONG).show();
        gString = new String[cursor.getCount()];

        for (int i=0;i<cursor.getCount();i++)
        {
            cursor.moveToPosition(i);
            gString[i] = String.valueOf(Uri.fromFile(new File(cursor.getString(cursor.getColumnIndex(MediaStore.Images.Media.DATA)))));
        }

        
        // Create custom adapter for listview
        adapter=new LazyImageLoadAdapter(this, gString);
